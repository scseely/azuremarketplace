...
APPINSIGHTS_INSTALLED=$(az extension list-available --query "[?name == 'application-insights']" | jq '.[] | .installed')
echo $APPINSIGHTS_INSTALLED 
...
APPINSIGHTS_KEY=$(az monitor app-insights component show -g $RESOURCE_GROUP --query "[?applicationId == '$APPID']" | jq '.[] | .instrumentationKey')
az logout
